...
@objcMembers public class YYCalendar: UIViewController {
    ...
    var calendarType: CalendarType = .normal
    ...
    var minDate: Date = Date(timeIntervalSince1970: 0) // 조회 가능 최소 날짜
    var maxDate: Date = Date() // 조회 가능 최대 날짜
    ...
    public convenience init(langType type: LangType, date: String, minDate: String?, maxDate: String?, format: String, completion selectHandler: SelectHandler?) {
        self.init(langType: type, date: date, format: format, disableAfterToday: false, completion: selectHandler)
        self.calendarType = .limited

        // minDate 기본값은 1900년 01월 01일, maxDate 기본값은 오늘
